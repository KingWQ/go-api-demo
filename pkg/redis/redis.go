...
func NewClient(address, username, password string, db int) *RedisClient {
	//1. 初始化自定的 RedisClient 实例
	rds := &RedisClient{}

	//2. 使用默认的context
	rds.Context = context.Background()

	//3. 使用 redis 库里的 NewClient 初始化连接
...
// Has 判断一个 key 是否存在，内部错误和 redis.Nil 都返回false
...
// Del 删除存储在 redis 里的数据，支持多个key传参
